...
def main():
    ...
    def pde(x, y):
        ...
        return -epsilon * (dy_xx + dy_yy + dy_zz) + (tf.multiply(beta_1, dy_x) + tf.multiply(beta_2, dy_y) + tf.multiply(beta_3, dy_z)) + gamma*y -\
               (-epsilon*(2*Y**2*Z**2+X**2*2*Z**2+X**2*Y**2*2) + (beta_1*2*X*Y**2*Z**2 + beta_2*2*Y*X**2*Z**2 + beta_3*2*Z*Y**2*X**2) + gamma*X**2*Y**2*Z**2)
    ...
    data = dde.data.PDE(geom, pde, bc, num_domain=100,
                        num_boundary=100, num_test=100, wight=[1, 10, 0])

    outer = scio.loadmat('./generating/outer_c_n_h_0.1.mat')
    outer = outer['p']
    inner = np.load("./generating/inner.npy")
    data.train_x = np.vstack((inner, outer))
    data.num_boundary = outer.shape[0]
    data.num_domain = inner.shape[0]
    data.num_test = data.train_x.shape[0]
    data.test_x = np.vstack((inner, outer))
    ...
    value = model.predict(data.test_x)
    value = np.hstack((data.test_x, value))
    scio.savemat('./generating/value.mat', {'value': value})
    ...
    #     true_value.append(true_fun(data.test_x[i, 0], data.test_x[i, 1], data.test_x[i, 2]))
    # tv_z = np.array(true_value)
    # scio.savemat('./generating/true_value.mat', {'value': tv_z})
